...
class ListaContatosMelhoradaFragment: Fragment(), SearchView.OnQueryTextListener {
    ...
    private fun carregaLista() {
        ...
        if (listaOrdemAlfabetica) {
            val listaOrdenada = AgendaV5.listaContatos.sortedBy { it.nome }
            adapter.swapData(listaOrdenada)
        } else {
            adapter.swapData(AgendaV5.listaContatos)
        }
    }
    ...
    override fun onQueryTextSubmit(query: String?): Boolean {
        ...
        val listaFiltrada = AgendaV5.listaContatos.filter { contatoAtual ->
